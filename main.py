...
def generate_resume(file_path, json_file_path, debug):
    # Create the PDF document
    doc = SimpleDocTemplate(file_path, pagesize=A4, showBoundary=0, 
                            leftMargin=0.5 * inch, rightMargin=0.5 * inch, 
                            topMargin=0.2 * inch, bottomMargin=0.1 * inch, 
                            title="Resume", author="Author")
    
    # Load data from JSON file
    with open(json_file_path, 'r') as file:
        data = json.load(file)

    # Extract name and contact details from JSON
    ...
    table_data.append([Paragraph(f"{email} | {phone} | {address}", CONTACT_PARAGRAPH_STYLE)])
    ...
    # Append experience
    for job_experience in data.get('experience', []):
        table_data.append([
            Paragraph(job_experience.get('company', ''), COMPANY_HEADING_PARAGRAPH_STYLE),
            Paragraph(job_experience.get('duration', ''), COMPANY_DURATION_PARAGRAPH_STYLE),
        ])
        table_styles.append(('TOPPADDING', (0, running_row_index), (1, running_row_index), 5))
        running_row_index += 1

        table_data.append([
            Paragraph(job_experience.get('title', ''), COMPANY_TITLE_PARAGRAPH_STYLE),
            Paragraph(job_experience.get('location', ''), COMPANY_LOCATION_PARAGRAPH_STYLE),
        ])
        table_styles.append(('TOPPADDING', (0, running_row_index), (1, running_row_index), 1))
        running_row_index += 1

        for line in job_experience.get('description', []):
            table_data.append([
    ...
    for education in data.get('education', []):
        table_data.append([
            Paragraph(education.get('university', ''), COMPANY_HEADING_PARAGRAPH_STYLE),
            Paragraph(education.get('year', ''), COMPANY_DURATION_PARAGRAPH_STYLE),
        ])
        table_styles.append(('TOPPADDING', (0, running_row_index), (1, running_row_index), 5))
        running_row_index += 1

        table_data.append([
            Paragraph(education.get('degree', ''), COMPANY_TITLE_PARAGRAPH_STYLE),
            Paragraph(education.get('location', ''), COMPANY_LOCATION_PARAGRAPH_STYLE),
        ])
        table_styles.append(('TOPPADDING', (0, running_row_index), (1, running_row_index), 1))
        running_row_index += 1

    # Append projects heading
    table_data.append([Paragraph("Projects", SECTION_PARAGRAPH_STYLE)])
    appendSectionTableStyle(table_styles, running_row_index)
    running_row_index += 1

    for project in data.get('projects', []):
        table_data.append([
            Paragraph(f"<font face='Garamond_Semibold'>{project.get('title', '')}: </font>{project.get('description', '')} {project.get('link', '')}", bulletText='•', style=JOB_DETAILS_PARAGRAPH_STYLE),
        ])
        table_styles.append(('TOPPADDING', (0, running_row_index), (1, running_row_index), 1))
        table_styles.append(('BOTTOMPADDING', (0, running_row_index), (1, running_row_index), 0))
        table_styles.append(('SPAN', (0, running_row_index), (1, running_row_index)))
...
# Ensure the `appendSectionTableStyle` and style definitions are correctly defined
...
if __name__ == "__main__":
    ...
    generate_resume(OUTPUT_PDF_PATH, JSON_PATH, debug)